...
public class CETTopicCache
{
		...
		if (!isShutdown && allNews != null)
		{
			for (final NewsInfo info : allNews)
			{
				executor.execute(new Runnable()
				{
					...
					public void run()
					{
						if (info.getCet_topics() != null && info.getCet_topics().length() > 10)
						{
							return;
						}
						if (BasicStringUtil.isNotNullString(info.getHtml_content()))
						{
							...
							String newContent = splitArticle(info.getHtml_content(), allFind);
							if (allFind.size() > 0)
							{
	...
	protected String deal(String s, List<Topic> keys)
	{
		for (Topic key : keys)
		{
			...
			s = s.replaceAll("([^a-zA-Z])" + matched_word + "([^a-zA-Z])", "$1<a href=\"http://m.iciba.com/" + matched_word + "\">" + matched_word + "</a>$2");
			...
			s = s.replaceAll("([^a-zA-Z]|^)" + matched_word2 + "([^a-zA-Z]|$)", "$1<a href=\"http://m.iciba.com/" + matched_word2 + "\">" + matched_word2 + "</a>$2");
		}
		return s;
	}
