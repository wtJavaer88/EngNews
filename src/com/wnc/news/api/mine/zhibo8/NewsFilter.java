...
public class NewsFilter
{
	...
	public static List<Zb8News> filterOutSide(List<Zb8News> newsList)
	{
		...
		for (Zb8News zb8News : newsList)
		{
			...
			if (BasicStringUtil.isNotNullString(from_url)
					&& !NewsUrlUtil.isZhibo8Url(from_url))
			{
				for (Entry<String, String> entry : WebSiteClassFactory
						.getWebsites().entrySet())
				{
	...
	public static List<Zb8News> filterKeyWord(List<Zb8News> newsList,
			String keyword)
	{
		List<Zb8News> ret = new ArrayList<Zb8News>();
		for (Zb8News zb8News : newsList)
		{
			if (zb8News.getKeyword() != null
					&& zb8News.getKeyword().contains(keyword))
			{
				ret.add(zb8News);
			}
		}
