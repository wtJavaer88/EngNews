...
public class Zb8CommentsUtil
{
	...
	public static List<Comment> getHotComments(String article_url) throws Exception
	{
		...
		String hotcommentsApiUrl = String.format(hot_format, getArticleDate(article_url), getArticleType(article_url), getArticleId(article_url));
		String jsonResult = "";
		try
		{
		...
		catch (Exception e)
		{
			...
			{
				hotcommentsApiUrl = String.format(hot_format, getYesterday(getArticleDate(article_url)), getArticleType(article_url), getArticleId(article_url));
				jsonResult = JsoupHelper.getJsonResult(hotcommentsApiUrl);
			}
		}
	...
	{
		String hotcommentsApiUrl = String.format(count_format, getArticleDate(article_url), getArticleType(article_url), getArticleId(article_url));
		String jsonResult = "";
		try
		{
			jsonResult = JsoupHelper.getJsonResult(hotcommentsApiUrl);
		...
		catch (Exception e)
		{
			if (e.getMessage().contains("404 error "))
			{
				hotcommentsApiUrl = String.format(count_format, getYesterday(getArticleDate(article_url)), getArticleType(article_url), getArticleId(article_url));
				jsonResult = JsoupHelper.getJsonResult(hotcommentsApiUrl);
			}
		}
