...
public class TestZb8
{
	...
	public void m() throws Exception
	{
		...
		List<Zb8News> filterTeam = NewsFilter.filterKeyWord(
	...
	public void testDm() throws Exception
	{
		...
		String extractNewsContent = new HtmlContentHelper()
	...
	{
		List<Zb8News> nbaNewsByDay = new NewsExtract()
				.getNBANewsByDay("2017-01-13");
		List<Zb8News> filterTeam = NewsFilter.filterWebSite(
				NewsFilter.filterKeyWord(nbaNewsByDay, "马刺"), "twitter");
		for (Zb8News zb8News : filterTeam)
		{
			System.out.println(zb8News.getTitle() + " / "
					+ zb8News.getFrom_url());
		...
		for (Zb8News zb8News : nbaNewsByDay)
		{
			...
			System.out.println("title:" + zb8News.getTitle() + "  url:"
					+ zb8News.getUrl());
			new HtmlContentHelper().initEngHtmlContent(zb8News);
			String eng_content = zb8News.getEng_content();
			if (eng_content != null)
				System.out
						.println(eng_content.substring(
								0,
								eng_content.length() > 100 ? 100 : eng_content
										.length()));
	...
	{
		List<Zb8News> newsByYearMonth = new NewsExtract().getNewsByYearMonth(
				2016, 7, SportType.NBA);
		for (Zb8News zb8News : newsByYearMonth)
		{
			...
			{
				System.out.println("find.." + zb8News.getTitle()
						+ zb8News.getUrl() + " / " + zb8News.getFrom_url());
			}
		}
	}
	...
	{
		String extractNewsContent = new HtmlContentHelper()
				.extractNewsContent("http://www.mirror.co.uk/sport/football/news/southampton-liverpool-player-ratings-karius-9605964");
		System.out.println(extractNewsContent);
	}
}
