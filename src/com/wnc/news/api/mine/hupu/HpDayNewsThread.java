...
public class HpDayNewsThread implements Runnable
{
	...
	public HpDayNewsThread(String lastUpdateTime, List<HpNews> list,
			SportType type)
	{
		this.type = type;
	...
	public void run()
	{
		if (lastUpdateTime.replace("-", "").substring(0, 8)
				.compareTo(BasicDateUtil.getCurrentDateString()) > 0)
		{
			return;
		}
	...
	private void getNews() throws Exception
	{
		...
		{
			Document doc = SoupUtil.getDoc(String.format(HP_URL_FORMAT,
					type.getDesc(), i));
			...
			for (Element element : select)
			{
				...
				if (other_left != null)
				{
					...
					// other_left.attr("title"));
					news.setNews_time(other_left.attr("title") + ":00");
					news.setDay(other_left.attr("title").replace("-", "")
							.substring(0, 8));
				}
				else
				{
			...
			System.out.println("page_" + i + "新闻条数:" + size);
		}
