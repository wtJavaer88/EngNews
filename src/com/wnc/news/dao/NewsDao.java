...
public class NewsDao
{
	...
	public synchronized static void deleteTestNews()
	{
		...
		{
			openDatabase();
			int delete = database.delete("news", "url like ?", new String[] { "%.realgm.%" });
	...
	public synchronized static void updateContentAndTopic(String url, String newContent, String cetTopics)
	{
		try
		{
			openDatabase();
	...
	public synchronized static void insertNews(List<NewsInfo> news)
	{
		try
		{
			...
			for (NewsInfo newsInfo : news)
			{
				try
				{
					database.execSQL("INSERT INTO NEWS(title,url,sub_text,date,head_pic,keywords,html_content,website_id) VALUES (?,?,?,?,?,?,?,?)", new Object[] { newsInfo.getTitle(), newsInfo.getUrl(), newsInfo.getSub_text(), newsInfo.getDate(), newsInfo.getHead_pic(),
							newsInfo.getKeywords().toString(), newsInfo.getHtml_content(), newsInfo.getWebsite().getDb_id() });
				}
				catch (Exception e)
				{
	...
	public static boolean isExistUrl(SQLiteDatabase db, String url)
	{
		...
		try
		{
			String sql = "select * from news where url='" + StringEscapeUtils.escapeSql(url) + "' order by date desc";
	...
	{
		return findAllNewsWithUrlFilter(".realgm.");
	}
	...
	public static List<NewsInfo> findAllNewsWithUrlFilter(String url_filter)
	{
		...
		String sql = "select * from news where 1=2 " + f + " order by date desc ";
	...
	{
		String sql = "select * from news where  html_content like '%" + keyword + "%' or  keywords like '%" + keyword + "%' order by date desc";
		return findAllNewsBySql(sql);
	}
