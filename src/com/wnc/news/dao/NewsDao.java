...
public class NewsDao
{
	...
	{
		database = DatabaseManager_Main.getInstance().openDatabase();
	}
	...
	public static List<NewsInfo> search(String keyword)
	{
		...
		for (String key : keys)
		{
			...
			if (webSiteId.length() == 0 && StaticsHelper.isWebSite(key))
			{
				webSiteId = StaticsHelper.getWebSiteId(key);
				episodeSql += " and website_id=" + webSiteId;
				continue;
			}
			if (team.length() == 0 && StaticsHelper.isTeam(key))
			{
				team = key;
