...
public class NewsContentUtil
{
	 ...
	 * @return
	 */
	public static String getSectionAndSetPos(SelectableTextView mTextView, int i, int j)
	{
		try
		{
			String content = mTextView.getText().toString();
			j--;
			while (i > -1 && i < content.length() && (content.charAt(i) + "").matches("[a-zA-z]{1}"))
			{
				i--;
			}
			i++;
			while (j > -1 && j < content.length() && (content.charAt(j) + "").matches("[a-zA-z]{1}"))
			{
				j++;
			}
	 ...
	 * @return
	 */
	public static String getSuitWordAndSetPos(SelectableTextView mTextView, int start)
	{
	...
	public static CharSequence getSection(String content, String selected)
	{
		...
		if (BasicStringUtil.isNull2String(content, selected) || content.length() < selected.length())
		{
			return "";
		}
		...
		try
		{
			while (i > -1 && i < content.length() && (content.charAt(i) + "").matches("[a-zA-Z0-9' ]{1}"))
			{
				i--;
			}
			i++;
			while (j > -1 && j < content.length() && (content.charAt(j) + "").matches("[a-zA-Z0-9' ]{1}"))
			{
				j++;
			}
			if (i >= j)
