...
public abstract class BaseNewsActivity extends BaseVerActivity implements CtrlableHorGestureDetectorListener, UncaughtExceptionHandler, OnClickListener
{
	// 最短承认的浏览时间
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		getLayoutInflater();
		main = LayoutInflater.from(this).inflate(R.layout.activity_news_content, null);
		hideVirtualBts();
		setContentView(main);
		initView();

		this.gestureDetector = new GestureDetector(this, new MyCtrlableGestureDetector(this, 0.5, 0, this, null));
	...
	protected Handler handler = new Handler()
	{
		...
		public void handleMessage(Message msg)
		{
			...
			case MESSAGE_ON_RUNTIME_TEXT:
				((TextView) findViewById(R.id.bt_activity_runtime)).setText(activityTimeUtil.getTranedRunTime());
				break;
			}
			dispatchMsg(msg);
		}
	...
	protected void recordViewed()
	{
		...
		System.out.println("recordViewed Runtime:" + activityTimeUtil.getTranedRunTime());
		if (activityTimeUtil.getRunTime() > VIEW_ADMIT_60 * 1000)
		{
			ActivityMgr.viewedNewsRecord(this, news_info, activityTimeUtil.getRunTime());
			kPIHelper.increaseViewed(activityTimeUtil.getRunTime(), allFind.size(), news_info);
		}
	}
	...
	protected void initView()
	{
		...
		newsMenuPopWindow = new NewsMenuPopWindow(this, new NewsMenuListener()
		{
			...
			public void toSrcPage()
			{
				if (news_info != null)
				{
					ActivityMgr.gotoIE(BaseNewsActivity.this, news_info.getUrl());
				}
			}
		...
		wordMenuPopWindow = new WordMenuPopWindow(this, new WordMenuListener()
		{
			...
			public void doSound()
			{
				...
				final String voicePath = MyAppParams.VOICE_FOLDER + getCurrentWord() + ".mp3";
				...
				else
				{
					...
					messagePopWindow.setMsgAndShow("找不到声音文件,正在为你下载!", mTextView);
					...
					new Thread(new Runnable()
					{
						...
						public void run()
						{
							...
							{
								Thread.sleep(1000);
								final String soundUrl = CibaCacheHelper.getCibaTranslate(getCurrentWord()).getSoundStr();
			...
			public void toNet()
			{
				ActivityMgr.gotoIE(BaseNewsActivity.this, WebUrlHelper.getWordUrl(getCurrentWord()));
			}
			...
			{
				final Dialog dialog = new Dialog(BaseNewsActivity.this, R.style.CustomDialogStyle);
				dialog.setContentView(R.layout.topic_tip_wdailog);
				dialog.setCanceledOnTouchOutside(true);
				Window window = dialog.getWindow();

				WindowManager.LayoutParams lp = window.getAttributes();
				lp.width = (int) (0.85 * BasicPhoneUtil.getScreenWidth(BaseNewsActivity.this));
				lp.height = (int) (0.85 * BasicPhoneUtil.getScreenHeight(BaseNewsActivity.this));
				lp.verticalMargin = 20;

				final SelectableTextView tvTopic = (SelectableTextView) dialog.findViewById(R.id.tvTopicInfo);
				...
				tvTopic.setOnLongClickListener(new View.OnLongClickListener()
				{
					...
					{
						tvTopic.setMovementMethod(ScrollingMovementMethod.getInstance());

						int start = tvTopic.getPreciseOffset(mTouchX2, mTouchY2);

						if (start > -1)
						{
							newsStack.push(ViewNewsHolder.getCurList());
							String selectedText = NewsContentUtil.getSuitWordAndSetPos(tvTopic, start);
							log.info("selectedText:" + selectedText);
							ClipBoardUtil.setNormalContent(getApplicationContext(), selectedText);
							startActivityForResult(new Intent(getApplicationContext(), SearchActivity.class).putExtra("keyword", selectedText), REQUEST_SEARCH_CODE);
						}
						return true;
					}
				});
				...
				{
					final Integer topic_id = OptedDictData.getSeekWordList().peek().getTopic_id();
					if (OptedDictData.getWordExpandContentMap().containsKey(topic_id))
					{
						tvTopic.setText(OptedDictData.getWordExpandContentMap().get(topic_id));
						dialog.show();
					}
					else
					{
						WordExpand wordExpand = DictionaryDao.findSameAntonym(OptedDictData.getSeekWordList().peek().getTopic_id());

						if (wordExpand != null)
						{
							try
							{
								String article = "<p>" + wordExpand.toString().replace("\n", "<br>") + "</p>";
								String splitArticle = new CETTopicCache().splitArticle(article, new ArrayList<Topic>());
								final CharSequence charSequence = new HtmlRichText(splitArticle).getCharSequence();
								OptedDictData.addWordExpand(topic_id, charSequence);
						...
						{
							ToastUtil.showShortToast(getApplicationContext(), "找不到扩展的内容!");
						}
					}
				}
		...
		sectionPopWindow = new SectionPopWindow(this, new WordSectionListener()
		{
			...
			{
				ActivityMgr.gotoIE(BaseNewsActivity.this, WebUrlHelper.getTranslateUrl(getCurrentSectionAndSetPos()));
		...
		mTextView.setOnCursorStateChangedListener(new OnCursorStateChangedListener()
		{
			...
			public void onDragStop(View v)
			{
				...
				if (selectedText.contains(" "))
				{
					...
					final int h = common.app.BasicPhoneUtil.getScreenHeight(BaseNewsActivity.this);
	...
	{
		boolean flag = true;
		final boolean onTouchEvent = this.gestureDetector.onTouchEvent(paramMotionEvent);
		if (!onTouchEvent)
		{
	...
	protected void onPause()
	{
		...
		System.out.println("onPause Runtime:" + activityTimeUtil.getTranedRunTime());
		...
		if (hasTopics())
		{
			...
			else
			{
				Collections.sort(allFind2, new Comparator<Topic>()
				{
					...
					{
						return arg0.getMatched_word().compareToIgnoreCase(arg1.getMatched_word());
					}
				});
			}
			Iterator<Topic> iterator = allFind2.iterator();
			String tpContent = "总词数/生词率: " + totalWords + "/" + BasicNumberUtil.convertScienceNum(100.0 * allFind2.size() / totalWords, 1) + "%\n";
	...
	protected void showWordZone(final String selectedText)
	{
		...
		{
			if (selectedText.equalsIgnoreCase(d.getBase_word()) || selectedText.equalsIgnoreCase(d.getWord_done()) || selectedText.equalsIgnoreCase(d.getWord_er()) || selectedText.equalsIgnoreCase(d.getWord_est()) || selectedText.equalsIgnoreCase(d.getWord_ing())
					|| selectedText.equalsIgnoreCase(d.getWord_past()) || selectedText.equalsIgnoreCase(d.getWord_pl()) || selectedText.equalsIgnoreCase(d.getWord_third()))
			{
				findWord = d;
				util.finishRun();
		...
		else
		{
			...
			new Thread(new Runnable()
			{
				...
				public void run()
				{
					try
					{
						String basicInfo = CibaCacheHelper.getCibaTranslate(getCurrentWord()).getBasicInfo();
