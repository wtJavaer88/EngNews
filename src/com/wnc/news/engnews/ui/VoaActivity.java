...
public class VoaActivity extends BaseVerActivity implements OnClickListener, UncaughtExceptionHandler, CtrlableDoubleClickGestureDetectorListener
{
	...
	private void initData()
	{
		...
		for (int i = 0; i < infos.size(); i++)
		{
			...
			int stoptime = (i == (infos.size() - 1) ? seektime : (infos.get(i + 1).getTime() * 100));
			mTextView.append(new ClickableRichText("播放", this, seektime, stoptime).getCharSequence());
			mTextView.append("  ");
			final ArrayList<Topic> allFind2 = new ArrayList<Topic>();
			mTextView.append(new HtmlRichText(new CETTopicCache().splitArticle(vinfo.getEn(), allFind2)).getCharSequence());
			mTextView.append("\n" + vinfo.getCh() + "\n\n");
			...
			totalWords += PatternUtil.getAllPatternGroup(vinfo.getEn(), "['\\w]+").size();
		}
		if (allFind.size() > 0)
		{
	...
	private void initView()
	{
		...
		mTextView.setOnCursorStateChangedListener(new OnCursorStateChangedListener()
		{
			...
			public void onDragStop(View v)
			{
				...
				if (selectedText.contains(" "))
				{
					...
					final int h = common.app.BasicPhoneUtil.getScreenHeight(VoaActivity.this);
		...
		wordMenuPopWindow = new WordMenuPopWindow(this, new WordMenuListener()
		{
			...
			public void toNet()
			{
				ActivityMgr.gotoIE(VoaActivity.this, WebUrlHelper.getWordUrl(getCurrentWord()));
			}
			...
			public void doExpand()
			{
				...
				final Dialog dialog = new Dialog(VoaActivity.this, R.style.CustomDialogStyle);
				dialog.setContentView(R.layout.topic_tip_wdailog);
				dialog.setCanceledOnTouchOutside(true);
				Window window = dialog.getWindow();

				WindowManager.LayoutParams lp = window.getAttributes();
				lp.width = (int) (0.85 * BasicPhoneUtil.getScreenWidth(VoaActivity.this));
				lp.height = (int) (0.85 * BasicPhoneUtil.getScreenHeight(VoaActivity.this));
					...
					else
					{
						...
						if (wordExpand != null)
						{
							try
							{
								...
								OptedDictData.getWordExpandContentMap().put(topic_id, charSequence);
		...
		sectionPopWindow = new SectionPopWindow(this, new WordSectionListener()
		{
			...
			{
				ActivityMgr.gotoIE(VoaActivity.this, WebUrlHelper.getTranslateUrl(getCurrentSectionAndSetPos()));
	...
	public void play(int seekTime, int stopTime)
	{
		...
		String mp3Id = PatternUtil.getLastPattern(news_info.getMp3().toLowerCase().replace("mp3", ""), "\\d+");
			...
			if (!isDownLoading)
			{
				...
				Thread downThread = new Thread(new Runnable()
				{
					...
					public void run()
					{
						try
						{
							int download = UrlPicDownloader.download(news_info.getMp3(), path);
							log.info(news_info.getMp3() + "  download..." + download);
							isDownLoading = false;
		...
		if (BasicFileUtil.isExistFile(path) && BasicFileUtil.getFileSize(path) > 1000)
		{
			playThread = new PlayThread(path, seekTime, stopTime, isAutoStop);
