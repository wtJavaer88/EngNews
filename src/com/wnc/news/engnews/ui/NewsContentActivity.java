...
public class NewsContentActivity extends Activity implements UncaughtExceptionHandler, OnClickListener
{
	...
	Handler handler = new Handler()
	{
		...
		private void splitArticle(Elements elements)
		{
			for (Element element : elements)
			{
				...
				Set<Topic> set = DictionaryDao.findCETWords(dialog.replace("target=\"_blank\"", ""));
				if (set.size() == 0)
				{
					mTextView.append(new WebImgText(dialog).getCharSequence());
				}
				else
				{
					for (Topic topic : set)
		...
		private String getDealResult(String aString, Set<Topic> keys)
		{
			...
			// 给关键词加上超链接
			mTextView.append(new WebImgText(result.toString()).getCharSequence());
		...
		private String deal(String s, Set<Topic> keys)
		{
			for (Topic key : keys)
			{
				s = s.replace(key.getMatched_word(), "<a href=\"http://m.iciba.com/" + key.getMatched_word() + "\" style=\"color:red;font-size:14px\">" + key.getMatched_word() + "</font></a>");
			}
			return s;
		}
	};
	...
	private void showSelectionCursors(int x, int y)
	{
		...
		if (start > -1)
		{
			...
			CharSequence selectedText = mTextView.getCursorSelection().getSelectedText();
			System.out.println(selectedText);
			DicWord findWord = DictionaryDao.findWord(selectedText.toString());
			if (findWord != null)
			{
				System.out.println(findWord);
				((TextView) findViewById(R.id.tv_oneword_tip)).setText(findWord.getBase_word() + " " + findWord.getCn_mean());
			}
		}
	}
