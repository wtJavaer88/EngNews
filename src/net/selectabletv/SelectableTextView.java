...
public class SelectableTextView extends TextView
{
    ...
    public void setSelection(int color, int start, int length, int duration)
    {
        ...
        /*
         * My Note have to create a new one each time instead of using the
         * {@code set} method. if I use set, will mess up the removeSelection
         * function as the content of the reference has changed
         */
        mCursorSelection = new SelectionInfo(getText(),
     ...
     * 
     * @param onCursorStateChangedListener
     *            the OnCursorVisibilityChangedListener
     */
    ...
    private int getScrollYInternal()
    {
        ...
        if (this.getParent() instanceof ScrollView)
        {
            ...
            /*
             * My Note getLocationInWindow(coords) for the TextView will return
             * a negative value if we scroll down as the top the TextView is
             * beyong the top of the screen.
             * 
             * So maybe we could replace SrollView.getScrollY() with
             * getWindowsInLocation(coords)
             */
        }
    ...
    /**
     * the current x position of the TextView taking into account the scroll (if
     * it's inside a ScrollView) and the padding of the scrollview
     * 
    ...
    private int getHysteresisOffset(int x, int y, int previousOffset)
    {
        ...
        // The "HACK BLOCK"S in this function is required because of how Android
        // Layout for
        ...
        // by 1 to compensate for the change made below. (see previous HACK
        // BLOCK)
        // ///////////////////HACK
        // BLOCK///////////////////////////////////////////////////
     ...
     */
    private void getAdjusteStartXY(int offset, int scroll_x, int scroll_y,
            int[] coords)
    {
     ...
     *            the returned x, y coordinate array, must have a length of 2
     */
    private void getAdjustedEndXY(int offset, int scroll_x, int scroll_y,
            int[] coords)
    {
        if (offset > 0)
        {
            ...
            ViewTreeObserver.OnTouchModeChangeListener
    {
        ...
        public void snapToSelection()
        {
            ...
            if (mIsShowing)
            {
                ...
                SelectableTextView.this.getAdjusteStartXY(start, scroll_x,
                        scroll_y, coords);
                startHandle.pointTo(coords[0], coords[1]);

                SelectableTextView.this.getAdjustedEndXY(end, scroll_x,
                        scroll_y, coords);
        ...
        public void show(int start, int end)
        {
            ...
            SelectableTextView.this.getAdjusteStartXY(a, scroll_x, scroll_y,
                    coords);
            mStartHandle.show(coords[0], coords[1]);

            SelectableTextView.this.getAdjustedEndXY(b, scroll_x, scroll_y,
                    coords);
            ...
            if (mOnCursorStateChangedListener != null)
            {
                mOnCursorStateChangedListener
                        .onShowCursors(SelectableTextView.this);
            }
        }
        ...
        public void hide()
        {
            if (mIsShowing)
            {
                ...
                {
                    mOnCursorStateChangedListener
                            .onHideCursors(SelectableTextView.this);
                }
            }
        }
                ...
                int oldx, int oldy)
        {
            ...
            int old_offset = cursorHandle == mStartHandle ? SelectableTextView.this
                    .getCursorSelection().getStart() : SelectableTextView.this
                    .getCursorSelection().getEnd();

            int offset = SelectableTextView.this.getHysteresisOffset(x, y,
                    old_offset);
            ...
            if (offset != old_offset)
            {
                ...
                {
                    SelectableTextView.this.getCursorSelection().setStart(
                            offset);
                }
                else
                {
            ...
            if (mOnCursorStateChangedListener != null)
            {
                mOnCursorStateChangedListener.onPositionChanged(
                        SelectableTextView.this, x, y, oldx, oldy);
            }
        }
        ...
        {
            SelectableTextView.this.setSelection(Math.min(start, end),
                    Math.abs(end - start));
        }
        ...
        /**
         * Called when the view is detached from window. Perform house keeping
         * task, such as stopping Runnable thread that would otherwise keep a
         * reference on the context, thus preventing the activity from being
         * recycled.
         */
        @SuppressWarnings("unused")
    ...
    private class CursorHandle extends View
    {
        ...
        public CursorHandle(SelectionCursorController controller)
        {
            ...
            /*
             * My Note At first I tried using mContainer = new
             * PopupWindow(SelectableTextView.this.getContext()) and
             * mContainer.setContentView(this) in the show() method AND FAILED
             * to draw the PopupWindow properly (e.g. PopupWindow can't contain
             * the whole drawable, background of the PopupWindow is not
             * transparent. I think it's because calling new
             * PopupWindow(context) uses the internal's default style which
             * messes up
             */
            ...
            case MotionEvent.ACTION_DOWN:
            {
                ...
                if (SelectableTextView.this.mOnCursorStateChangedListener != null)
                {
                    SelectableTextView.this.mOnCursorStateChangedListener
                            .onDragStarts(SelectableTextView.this);
                }
                break;
            }
        ...
        public void show(int x, int y)
        {
            ...
            mContainer.showAtLocation(SelectableTextView.this,
                    Gravity.NO_GRAVITY, coords[0], coords[1]);
        }
        ...
        private void onDetached()
        {
            ...
            // Since no action windows is associate with the cursor, nothing is
            // removed.
            // I've made the action window a task of the View using
            // SelectableTextView instead of
...
/*
 * Note for future me: The "LINE" for layout uses 0 based index e.g.
 * getLineForVertical
 */
